...
function result_Fitness = unimyFitness ( x, Fit_paras )
...
%update W
 ...
 W_next = uniFAUpdateWeight_FCAPSO(CBPE, Fit_paras.sW_Current, delta_g); %整个权重矩阵应该就是一个数
 W_NextIter = W_next * ones(size_particle, dim_objfunction, 1);
...
%update velocity
    ...
    for i = 1:size_particle
        if ( F_Next(i) < Fit_paras.sLBest_Curr(i) )
            %如果最优值小了，更新            
            result_Fitness = result_Fitness + 1 +  W_next;   
             %如果最优值小了，更新            
